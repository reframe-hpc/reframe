...
def test_inline_str_assert(local_exec_ctx):
    class MyTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello'
        sanity_patterns = r'hello'

    _run(MyTest(), *local_exec_ctx)


def test_decorated_sanity(local_exec_ctx):
    class MyTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello'
...
def test_sanity_patterns_raises_error(local_exec_ctx):
    class MyTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello, world!'
        sanity_patterns = r'ehllo'

        @sanity_function
        def assert_world(self):
            return sn.assert_found(r'world', self.stdout)

    with pytest.raises(SanityError, match="pattern 'ehllo' not found"):
        _run(MyTest(), *local_exec_ctx)


def test_decorated_sanity_raises_error(local_exec_ctx):
    class MyTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello, world!'
        ...
        @sanity_function
        def misspelled_assert(self):
            return sn.assert_found(r'wodrl', self.stdout)

    with pytest.raises(SanityError, match="pattern 'wodrl' not found"):
        _run(MyTest(), *local_exec_ctx)


def test_inherit_decorated_sanity(local_exec_ctx):
    class MyBaseTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello, world!'
    ...
    class Foo(MyBaseTest):
        @sanity_function
        def assert_hello(self):
            return sn.assert_found(r'hello, world!', self.stdout)

    class Bar(Foo):
        '''Trigger error from the ``assert_hello`` function.'''
        executable = 'echo hello, wodrl!'

    with pytest.raises(SanityError, match="pattern 'wodrl' not found"):
        _run(Foo(), *local_exec_ctx)

    with pytest.raises(SanityError, match="pattern 'hello, world!' not found"):
        _run(Bar(), *local_exec_ctx)


def test_override_inherited_decorated_sanity(local_exec_ctx):
    class MyBaseTest(rfm.RunOnlyRegressionTest):
        valid_prog_environs = ['*']
        valid_systems = ['*']
        executable = 'echo hello, world!'
    ...
    class Foo(MyBaseTest):
        ...
        @sanity_function
        def another_misspelled_assert(self):
            return sn.assert_found(r'hello, wodrl!', self.stdout)

    class Bar(Foo):
        '''Trigger error from ``misspelled_assert``.'''
        executable = 'echo hello, wodrl!'

    with pytest.raises(SanityError, match="pattern 'hello, wodrl!' not found"):
        _run(Foo(), *local_exec_ctx)

    with pytest.raises(SanityError, match="pattern 'world!' not found"):
        _run(Bar(), *local_exec_ctx)
