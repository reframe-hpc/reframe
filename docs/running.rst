...
A report of the performance values extracted during the regression tests can be generated by running ReFrame with the command line option `--performance-report`.

The performance report is printed after the output of the regression and has the following format.
...
where the performance variables are listed by programming environment and partition for each of the tests that are run.

For instance

  .. code-block:: bash
  reframe -C tutorial/config/settings.py -c tutorial/example7.py -r --performance-report

prints something like this

  .. code-block:: bash

  Command line: ./reframe -r -c tutorial/example7.py --performance-report
...
If a test needs to be run on a situation where there are no references, the `self.reference` dictionary may be written as
...
This will pass the performance checks and the performance report will be printed.
