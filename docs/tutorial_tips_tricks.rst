...
If the test is not failing but the user still wants to experiment or modify the existing :attr:`sanity_patterns` or :attr:`perf_patterns`, the user can pass the command line argument ``--keep-stage`` when running ReFrame to avoid deleting the stage directory.
