...
for data in fs.values():
    if 'ior_block_size' not in data:
        data['ior_block_size'] = '24g'
    if 'ior_access_type' not in data:
...
class IorCheck(rfm.RegressionTest):
    def __init__(self, base_dir):
        ...
        self.valid_systems = fs[base_dir]['valid_systems']

        cur_sys = self.current_system.name

        try:
            self.num_tasks = fs[base_dir][cur_sys]['num_tasks']
        except KeyError:
            self.num_tasks = 1
        try:
            tasks_per_node = fs[base_dir][cur_sys]['num_tasks_per_node']
            self.num_tasks_per_node = tasks_per_node
        except KeyError:
            self.num_tasks_per_node = 1

        self.ior_block_size = fs[base_dir]['ior_block_size']
        self.ior_access_type = fs[base_dir]['ior_access_type']
        ...
        try:
            prog_env = fs[base_dir][cur_sys]['valid_prog_environs']
            self.valid_prog_environs = prog_env
        except KeyError:
            self.valid_prog_environs = ['PrgEnv-cray']

        try:
            self.build_system.cc = fs[base_dir][cur_sys]['build_system_cc']
        except KeyError:
            pass
        try:
            self.build_system.cxx = fs[base_dir][cur_sys]['build_system_cxx']
        except KeyError:
            pass
