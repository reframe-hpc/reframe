  ...
  while (end == 0)
      ...
      if (end == 0)
	{
	  ...
	  if (color == 0)
	    {
	      if (MPI_Comm_split
		  (MPI_COMM_WORLD, MPI_UNDEFINED, comm_rank, &red_comm) != 0)
		{
	      ...
	      for (j = 0; j < NCALLS; j++)
		{
		  for (i = 0; i < ndims; i++)
			  ...
			  (cart_comm, i, 1, &rank_source, &rank_dest) != 0)
			{
		      ...
		      if (MPI_Irecv
			  (recvbuf + i * 2 * halosize[i] * sizeof (char),
		  ...
		  for (i = 0; i < ndims; i++)
			  ...
			  (cart_comm, i, 1, &rank_source, &rank_dest) != 0)
			{
		      ...
		      if (MPI_Isend
			  (sendbuf + i * 2 * halosize[i] * sizeof (char),
			   halosize[i], MPI_CHAR, rank_source, 1, cart_comm,
	      ...
	      if (MPI_Reduce
		  (&elapsed_time, &deltatmin, 1, MPI_DOUBLE, MPI_MIN, 0,
		   cart_comm) != 0)
		{
	      ...
	      if (MPI_Reduce
		  (&elapsed_time, &deltatmax, 1, MPI_DOUBLE, MPI_MAX, 0,
		   cart_comm) != 0)
		{
		  fprintf (stderr, "MPI_Reduce() failed\n");
		  exit (1);
